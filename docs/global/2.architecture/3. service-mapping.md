...
Chi tiết BC và business rules: `bounded-contexts.md`
...
|--------------------|-------------------------------------------------------------------------------------------------------------|-----------|
| `api-gateway`      | Entry point, routing thuần tới web-gateway/mobile-gateway/oauth2-service, rate limit per-IP                 | Always-on |
| `web-gateway`      | BFF cho 3 Angular app, session cookie, token exchange                                                       | Always-on |
| `mobile-gateway`   | BFF cho mobile client (Shipper app) — JWT resource server, aggregation, ACL, version gating, device binding | Always-on |
...
| `order-service`       | Order           | Order lifecycle, Saga choreography (CRUD — ADR-010)        | Always-on |
