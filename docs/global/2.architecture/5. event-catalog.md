...
| `SessionIssuedEvent`       | `oauth2.session.issued`                    | identity-service      | Projection | `authorizationId`, `oauthSessionId`, `idpSessionId`, `userId`, `deviceHash`, `userAgent`, `ipAddress`, `provider` |
| `LoginOtpRequested`        | `oauth2.login-otp.requested`               | notification-service  | Notify     | `userId`, `email`, `token`                                                                                |
| `LoginFailedEvent`         | `oauth2.login.failed`                      | identity-service      | Projection | `userId`, `loginIdentifier`, `result`, `deviceHash`, `ipAddress`, `userAgent`, `provider`                 |
| `PasswordSetupResentEvent` | `oauth2.credential.password-setup-resent`  | notification-service  | Notify     | `userId`, `email`, `setupToken`                                                                           |
| `SessionRevokedEvent`      | `oauth2.session.revoked`                   | identity-service      | Projection | `oauthSessionIds`, `userId`, `idpSessionId`                                                              |
| `SessionsBulkExpiredEvent` | `oauth2.session.expired.bulk`              | identity-service      | Projection | `oauthSessionIds`                                                                                         |

> `DeviceLoginRecorded` (tên trong ADR-001 gốc) đã đổi thành `SessionIssuedEvent` trong code thực tế — cùng mục đích (identity-service upsert `Device` + `LoginActivity`), catalog cập nhật theo tên hiện hành.
...
| Event                      | Topic                           | Consumers                              | Pattern    | Key Payload                                                                  |
|----------------------------|---------------------------------|----------------------------------------|------------|------------------------------------------------------------------------------|
| `ProductPublishedEvent`    | `catalog.product.published`     | search-service, inventory-service      | Projection | `productId`, `sellerId`, `categoryId`, `brandName`, `skuIds[]`, `name`       |
| `ProductUnpublishedEvent`  | `catalog.product.unpublished`   | search-service, inventory-service      | Projection | `productId`, `sellerId`                                                      |
| `ProductBlockedEvent`      | `catalog.product.blocked`       | search-service, inventory-service      | Projection | `productId`, `sellerId`, `reason`                                            |
| `ProductUnblockedEvent`    | `catalog.product.unblocked`     | inventory-service                      | Projection | `productId`                                                                  |
| `ProductUpdatedEvent`      | `catalog.product.updated`       | search-service                         | Projection | `productId`                                                                  |
| `VariantCreatedEvent`      | `catalog.variant.created`       | inventory-service                      | Projection | `skuId`, `productId`, `sellerId`, `active`, `productPublished`               |
| `VariantPriceChangedEvent` | `catalog.variant.price-changed` | search-service                         | Projection | `skuId`, `productId`, `newPrice` (long, VND)                                 |
| `VariantActivatedEvent`    | `catalog.variant.activated`     | search-service, inventory-service      | Projection | `skuId`, `productId`                                                         |
| `VariantDeactivatedEvent`  | `catalog.variant.deactivated`   | search-service, inventory-service      | Projection | `skuId`, `productId`                                                         |
| `CategoryUpdatedEvent`     | `catalog.category.updated`      | search-service                         | Projection | `categoryId`                                                                 |
...
| Event                    | Topic                  | Consumers                                                       | Pattern | Key Payload                                    |
|---------------------------|-------------------------|--------------------------------------------------------------------|---------|-------------------------------------------------|
| `ScheduledJobFiredEvent` | `scheduler.job.fired`  | `customer-service`, `cart-service`, `payment-service` (theo `taskType`) | Notify  | `scheduledJobId`, `instanceId`, `taskType`, `payload` |

`instanceId` — id của `ScheduledJobInstance` (aggregate riêng, 1 record cho mỗi lần fire) vừa dispatch cho lần fire này. Consumer mang nguyên `instanceId` khi callback báo outcome — không dùng `scheduledJobId` vì 1 recurring job tạo nhiều instance khác nhau theo thời gian.

### Consumes — callback outcome (contract phía scheduler-service, topic cụ thể do từng producer đặt tên)

`scheduler-service` consume 1 event khi consumer (`customer-service`/`cart-service`/`payment-service`) xử lý xong `ScheduledJobFiredEvent` — cập nhật `ScheduledJobInstance.status` (SUCCEEDED/FAILED), **không** ảnh hưởng `ScheduledJob.status`/`nextFireAt` (xem `service.md` §Callback). Contract tối thiểu — tên topic + tên event cụ thể do mỗi producer tự đặt theo convention của chính BC đó (`{producer}.{entity}.{past-tense-verb}`), **chưa chốt** — cần làm khi implement Phase 3+ của từng consumer:

| Field         | Type                 | Ghi chú                                        |
|----------------|----------------------|--------------------------------------------------|
| `instanceId`       | string               | Bắt buộc — khoá để load đúng `ScheduledJobInstance`  |
| `outcome`     | `SUCCEEDED \| FAILED` | Bắt buộc                                        |
| `failureReason` | string, nullable   | Chỉ có khi `outcome=FAILED`                     |
| `completedAt` | Instant              | Bắt buộc                                        |

**Khuyến nghị partition key = `instanceId`** cho mọi producer khi chốt topic — Kafka đảm bảo mọi message cùng `instanceId` (kể cả redelivery) vào cùng 1 partition, xử lý tuần tự bởi đúng 1 consumer, loại bỏ race 2 message cùng `instanceId` được xử lý đồng thời ở `scheduler-service`. Đây là khuyến nghị **cộng thêm**, không phải điều kiện đủ — `scheduled_job_instance.version` (`@Version`, xem `service/scheduler-service/data.md` §Locking) là lớp phòng thủ chính, tự scheduler-service kiểm soát và verify được, không phụ thuộc producer làm đúng convention này hay không.
...
Quyết định + rationale: `adr/011-dlq-per-service-strategy.md`. Cách implement (partition key, debug header, tránh vòng lặp): `3.technical/dlq-implementation-notes.md`.

**1 DLQ / consumer service**, trừ email-worker tách theo tier:
...
| oauth2-service | 1 | `oauth2-service.dlq` | Cao | 1 giờ |
