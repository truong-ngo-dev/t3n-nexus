...
Chi tiết topic, payload, consumers: `event-catalog.md`
...
**Ngoại lệ auth**: browser không có JWT (BFF pattern) nên WebSocket handshake không tự mang được credential. `web-gateway` expose 1 REST endpoint bình thường (`GET /web/webgw/auth/ws-ticket`, cookie auth) để mint ticket dùng 1 lần (Redis, TTL 10s) trước khi connect thẳng vào `websocket-gateway`. Đây chỉ là 1 REST call mint ticket — không phải proxy data-plane, nguyên tắc "không đi qua web-gateway" vẫn đúng cho toàn bộ connection lifetime sau đó. Chi tiết: `design/services/notification-service.md`.
