...
Service mapping: `service-mapping.md`. Architecture decisions: `adr/`.
...
- **CRUD trên Order aggregate** (không Event Sourcing — xem ADR-010) — audit trail dựa vào Kafka topic retention (`order.order.*`) qua Outbox, không replay per-event
