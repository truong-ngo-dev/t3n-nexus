...
| 3 | `global/2.architecture/event-catalog.md`       | Events đã có — không tạo duplicate            |
...
| `global/3.technical/5. dlq-strategy.md`      | Feature có Kafka consumer             |
...
- Events mới / payload thay đổi → `global/2.architecture/event-catalog.md`
- API mới → `service/{name}/api.yaml`
- Schema mới → `service/{name}/data.md`
- Flow thực tế khác design → `feature/{name}/design.md` + `sequence.puml`
- Quyết định kiến trúc mới → `global/2.architecture/adr/{n}-{slug}.md`
...
├── design.md          ← flow, actors, business rules, failure scenarios
├── sequence.puml      ← cross-service sequence
└── implementation.md  ← task checklist, thứ tự triển khai
```
...
|-----------------------------------------------|----------------------------------------------------------------------------------------|
| Thêm domain event mới                         | `global/2.architecture/event-catalog.md` + `service/{name}/service.md` > Domain Events |
| Sửa payload của event                         | `global/2.architecture/event-catalog.md`                                               |
...
| Flow thực tế khác với design                  | `feature/{name}/design.md` + `sequence.puml`                                           |
...
- [ ] Nếu flow thay đổi: `design.md` và `sequence.puml` đã cập nhật
