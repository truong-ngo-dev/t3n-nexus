...
**Sequence diagram (đã implement — `implementation.md` Phase 0-5):**
...
OS -> K : OrderConfirmed\n{orderId, customerId, sellerId, shippingAddress}
deactivate OS
...
**Sequence diagram**: chưa vẽ — thêm khi bắt đầu implement nhánh Prepaid.
...
**Sequence diagram — `INVENTORY_TIMEOUT` (3 phút, không phản hồi):**
...
`project_nfr.md`: checkout (sync response `POST /orders`) cần P99 < 2s; baseline write RPS toàn hệ thống ~800/s. Flash sale (200k user tranh 1 SKU) đi qua **pipeline riêng hoàn toàn** (`flashsale.md` — Bloom Filter + Redis Slot Gate, topic `inventory.flash-sale.requests` partition theo `skuId`), không chung code path với `POST /orders` thường ở đây — nên NFR của feature này không cần cover kịch bản hot-SKU, chỉ cần cover đúng baseline traffic + rủi ro do gap implementation hiện tại.

| Rủi ro                                                                            | Vì sao                                                                                                                                                                                                                                                                                               | Đề xuất                                                                                                                                                                                                                                                                                                                          |
|-----------------------------------------------------------------------------------|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| **`POST /orders` không có idempotency key/UNIQUE constraint**                     | Xác nhận đúng trên code (`implementation.md` Phase 1 caveat + Phase 2 migration — bảng `orders` không có cột idempotency key nào) — buyer double-click "Xác nhận", hoặc FE retry sau timeout mạng, tạo **2 Order thật** cho cùng 1 lần đặt hàng, cả 2 đều reserve tồn kho                            | Thêm `idempotency_key` (VD FE tự sinh UUID lúc mở trang checkout) + `UNIQUE` constraint trên `orders`, theo đúng pattern đã dùng cho `reservation.order_id` bên inventory-service (check-then-act + DB constraint làm lưới an toàn cuối, không dùng Redis lock — cùng lý do đã áp dụng cho idempotency Kafka consumer ở Phase 3) |
| **`customerId`/`sellerId` nhận thẳng từ request body, không qua JWT**             | Xác nhận đúng (`implementation.md` Phase 1 caveat, chưa giải quyết) — bất kỳ ai gọi `POST /orders` cũng có thể tự khai `customerId` bất kỳ, tạo đơn hàng gán cho người khác                                                                                                                          | Đã biết trước, kế hoạch giải quyết ở Phase 7-8 khi nối `web-gateway` tokenRelay thật — không phải gap mới, chỉ nhắc lại trong NFR để không quên trước khi go-live                                                                                                                                                                |
| **`CREATED`-timeout (Phase 5) chưa hoàn thiện — late-reply re-publish còn TODO**  | Nếu `inventory-service` phản hồi muộn sau khi Order đã timeout-cancel, `Reservation` có thể mồ côi giữ tồn kho vĩnh viễn (chi tiết: diagram `INVENTORY_TIMEOUT` + `implementation.md` Phase 5 "Quan trọng — quay lại sửa Phase 3") — ảnh hưởng trực tiếp tới tồn kho khả dụng, không phải chỉ lỗi UX | Phải xong trước khi coi Phase 5 là DONE — đã có trong checklist `implementation.md`, nhắc lại vì đây là rủi ro dữ liệu (orphaned reservation), không chỉ thiếu tính năng                                                                                                                                                         |
| **Gateway routing (Phase 8) chưa wire**                                           | `order-service` hiện chưa route được từ `web-gateway`/`api-gateway` — cùng loại gap đã gặp ở `catalog-service` trước khi sửa (thiếu route + có thể thiếu `SecurityFilterChain` tương ứng, chưa verify riêng cho order-service)                                                                       | Khi làm Phase 8, kiểm tra luôn `order-service` có `SecurityFilterChain` đúng chưa (permitAll không cần thiết ở đây vì mọi endpoint đều cần auth, nhưng vẫn cần đảm bảo JWT resource-server config đúng, không rơi vào 401 toàn bộ như đã gặp)                                                                                    |
| **Notification chain (Phase 6-7) chưa build — buyer chỉ có 1 đường nhận kết quả** | `notification-service` handler + `inapp-worker` chưa tồn tại — FE hiện tại **không có** WebSocket "OrderConfirmed" nào để nhận, chỉ còn cách poll `GET /orders/{orderId}` (fallback, không phải đường chính theo thiết kế)                                                                           | Không block P99 <2s của `POST /orders` (response đó độc lập, trả ngay `201` trước khi saga chạy xong) — nhưng UX "biết kết quả real-time" chưa hoạt động cho tới khi Phase 6-7 xong, cần biết trước khi demo                                                                                                                     |

Không có rủi ro riêng cho `InventoryReserved`/`InventoryReservationFailed` consumer (Phase 3) hay `inventory-service` idempotency (Phase 4) — cả 2 đã hardening đúng (`canProcess()` + `ObjectOptimisticLockingFailureException`, `DataIntegrityViolationException` catch + rollback), không phát hiện gap khi rà soát.
...
| `OrderConfirmed`             | `order-service`       | `fulfillment-service`, `notification-service` | `{ orderId, customerId, sellerId, shippingAddress }`           |
| `OrderCancelled`             | `order-service`       | `inventory-service`, `notification-service`   | `{ orderId, customerId, reason, cancelledBy }`                 |
