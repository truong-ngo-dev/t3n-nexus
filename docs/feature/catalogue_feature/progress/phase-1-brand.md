...
### Application — Queries
- [x] `ListActiveBrands` — `Query()`, `Result(List<BrandSummary>)`, `Handler`  
...
### Presentation
- [x] `presentation/brand/model/`: `CreateBrandRequest`, `UpdateBrandRequest`, `BrandResponse`
- [x] `BrandController`:
...
### 2026-06-14
- `ListActiveBrands`: Query + Handler với `@Cacheable(CacheNames.BRANDS_ACTIVE)`
- `CreateBrandRequest`, `UpdateBrandRequest`, `BrandResponse` records
- `BrandController`: 4 endpoints (GET /api/brands, POST/PUT/DELETE /api/admin/brands)
- Fix `CreateBrand`: đổi hardcoded `"brands:active"` → `CacheNames.BRANDS_ACTIVE`
